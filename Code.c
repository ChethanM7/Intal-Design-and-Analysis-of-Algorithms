...
char* intal_add(char* intal1, char* intal2) {
  int a1 = strlen(intal1);
  int a2 = strlen(intal2);

  int l = (a1 > a2)? a1 + 1: a2 + 1;
  int s = (a1 > a2)? a2: a1;
  ...
  for(i = 0 ; i < s; ++i) {
    d1 = convert_to_int(intal1[a1 - 1 - i]);
    d2 = convert_to_int(intal2[a2 - 1 - i]);
  ...
  while(i < l - 1) {
    if(a1 > a2) {
      rd = convert_to_int(intal1[a1 - 1 - i]) + m;
      m = (rd > 9)? 1: 0;
      rd = (rd > 9)? rd - 10: rd;

      result[l - 1 - i] = convert_to_char(rd);
    } else {
      rd = convert_to_int(intal2[a2 - 1 - i]) + m;
      m = (rd > 9)? 1: 0;
      rd = (rd > 9)? rd - 10: rd;
